...
MainWindow::MainWindow(ModbusManager *mgr, QWidget *parent)
    : QWidget(parent)
    , m_mgr(mgr)
{
    setWindowTitle(QStringLiteral("Modbus TCP Manager Demo (心跳/重连/队列/模拟故障)"));
    resize(860, 560);

    // ---- 连接参数 ----
    m_hostEdit = new QLineEdit(QStringLiteral("10.29.4.166"), this);
    ...
    for (int i = 0; i < 500; ++i) {
        ...
        const quint64 seq = m_mgr->enqueueRead(
            ...
            [this](bool ok, const QVector<quint16> &) {
                if (ok) ++m_okCount; else ++m_failCount;
                --m_busyReqCount;
            });
        if (seq != 0)
            ++m_busyReqCount;
